...
- **Never name an event `purchase`.** Whop generates its own `purchase` from payment data and forwards it to Meta as the conversion your ads optimize toward. Yours is not merged into it. It is accepted and recorded, but it sits beside Whop's under a near-identical name, and only Whop's feeds that optimization. Whop support confirms this and asks for a clearly different name. The full set Whop reserves for its own conversion types is `purchase`, `lead`, `schedule`, `submit_application`, `contact`, `complete_registration`, `view_content`, `add_to_cart`, `custom` and `messaging_conversation`. For a purchase on your own infrastructure, use something like `order_completed`.
